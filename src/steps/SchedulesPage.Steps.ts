    ...
    Then('Select schedule date {string},<\\{{int}}>,<\\{{int}}>', async function (this: PWWorld, month: string, year: number, day: number) {
        ...
        Then('Click search by dropdown', async function (this: PWWorld) {
            const schedulesPage = new SchedulesPage(this.page);
            await schedulesPage.getSearchByDropdownOptions(); 
        });


        Then('Select Route option in search by dropdown', async function (this: PWWorld) {
            const schedulesPage = new SchedulesPage(this.page);
